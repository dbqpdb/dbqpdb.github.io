...
module InspireHelpers
  def self.number_to_human(number)
    if number >= 1_000_000_000
      format("%.2gB", number / 1_000_000_000.0)
    elsif number >= 1_000_000
      format("%.2gM", number / 1_000_000.0)
    elsif number >= 1_000
      format("%.2gK", number / 1_000.0)
    else
      number.to_s
    end
  end
  ...
  class InspireHEPCitationsTag < Liquid::Tag
    ...
    def render(context)
      ...
      begin
        ...
        citation_count = InspireHelpers.number_to_human(citation_count)
